 ...
 **************************
 * @file           : pullup.c
 * @author         : Humberto Barrera, Catalina Muñoz, Erick Campos, Mauricio
 *Zavala
 * @board          : NUCLEO-F103RB
 **************************
 *
...
int main(void) {
  ...
  for (;;) {
    USER_Debounce();
    // GPIOA->BSRR	=	( 0x1UL <<  5U );//	value to reset pin5 of
    // port A (Turn-OFF LD2) GPIOA->BSRR	=	( 0x1UL << 21U );//
    // value to set pin5 of port A (Turn-ON LD2)
  }
}
...
void USER_Debounce(void) {
  if (!BUTTON) {
    USER_Delay();  // wait 10ms to avoid noise
    if (!BUTTON) { // double verification!
                   /* if pressed ('0') DO SOME TASK */
      GPIOA->ODR ^= (0x1UL << 5U);
      while (!BUTTON) {
      } // wait until button is released
      USER_Delay(); // wait another 10ms
    }
  }
}
...
void USER_GPIO_Init(void) {
  ...
  // PA10 as input pull up
  GPIOA->CRH &= ~(0x1UL << 10U) & ~(0x33UL << 8U);
